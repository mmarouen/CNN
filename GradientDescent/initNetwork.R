initNetwork<-function(K,dims,archi=NULL){
  ...
  if(is.null(archi)){ #default architecture
    conv=1
    fc=3
    archi=matrix(ncol=3*conv+fc,nrow = 5)
    colnames(archi)=c(rep(c("C","C","P"),conv),"F","F","F")
    rownames(archi)=c("Fsize_r","Fsize_l","Fstride_r","Fstride_l","Nnumber")
    archi[1,]=c(rep(c(3,3,2),conv),rep(NA,fc))
    archi[2,]=c(rep(c(3,3,2),conv),rep(NA,fc))
    archi[3,]=c(rep(c(1,1,2),conv),rep(NA,fc))
    archi[4,]=c(rep(c(1,1,2),conv),rep(NA,fc))
    # archi[5,]=c(8,8,NA,16,16,NA,32,32,NA,64,64,K)
    archi[5,]=c(8,8,NA,64,64,K)
