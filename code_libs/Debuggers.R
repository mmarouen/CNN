...
Objective<-function(Y,response,wD=NULL,lossFunction="crossEntropy"){
  K=ncol(Y)
  N=nrow(Y)
  eps=0
  objective=c()
  if(lossFunction=="crossEntropy"){
    if(K==1){objective=-mean(response*log(Y+eps)+(1-response)*log(1-Y+eps))}
    # if(K>1){objective=-mean(rowSums(response*log(Y+eps)))}
    if(K>1){objective=-mean(rowSums(response*log(Y+eps)))}
    
  }
  if(is.null(wD))wD=list(FALSE)
  if(wD[[1]]){
    lambda=wD[[2]]
    s=0
    for(i in 1:length(W)){
